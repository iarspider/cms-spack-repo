...
class Vecgeom(CMakePackage, CudaPackage):
    ...
    variant('vector', values=('auto', 'sse2', 'sse3', 'ssse3', 'sse4.1', 
                      ...
                      description='Vector instruction set to be used; ' +
                                  'auto lets Spack choose the backend',
                      default='auto')
    ...
    # Extra variants for vecgeom vectorization setting
    patch('vecgeom-fix-vector.patch')
    ...
    def cmake_args(self):
        if self.spec.satisfies('vector=auto'):
