...
class PyPoetry(PythonPackage):
    ...
    depends_on(
        "py-cleo@0.8.1:", type=("build", "run")
    )  # CMS: don't require 1.1.0a5 or newer
    ...
    depends_on(
        "py-virtualenv@20.4.3:20.4.5,20.4.7:", type=("build", "run"), when="@1.2:"
    )
    ...
    depends_on(
        "py-importlib-metadata@1.6:1", when="@:1.1 ^python@:3.7", type=("build", "run")
    )
    depends_on(
        "py-importlib-metadata@4.4:", when="@1.2: ^python@:3.9", type=("build", "run")
    )
    depends_on(
        "py-jsonschema", when="@1.2:", type=("build", "run")
    )  # CMS: loosen requirements
