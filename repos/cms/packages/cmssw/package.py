...
class Cmssw(ScramPackage):
    ...
    version("CMSSW_13_1_X_2023-02-13-2300", tag="CMSSW_13_1_X_2023-02-13-2300")
    version("CMSSW_13_0_X_2023-02-06-2300", tag="CMSSW_13_0_X_2023-02-06-2300")
    ...
    def edit(self, spec, prefix):
        ...
        if "_CXXMODULE_X" in str(spec.version):
            shutil.copy(
                join_path(os.path.dirname(__file__), "CXXModules.mk"),
                "config/SCRAM/GMake/CXXModules.mk",
            )

    @run_after("install")
    def cleanup_extra_data_dirs(self):
        for dirname in glob(
            join_path(self.spec.prefix, "external", self.cmsplatf, "data?*")
        ):
    ...
    def old_setup_dependent_environment(self, spack_env, run_env, dspec):
        ...
        spack_env.append_path(
            "LD_LIBRARY_PATH",
            self.prefix + "/" + cmssw_u_version + "/lib/" + self.scram_arch,
        )
