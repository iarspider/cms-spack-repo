...
class Cuda(Package):
    ...
    def install(self, spec, prefix):
        ...
        os.symlink(
            join_path(prefix.lib64.stubs, "libcuda.so"),
            join_path(prefix.lib64.stubs, "libcuda.so.1"),
        )
        install(
            join_path(prefix.build.lib64.stubs, "libnvidia-ml.so"), prefix.lib64.stubs
        )
        os.symlink(
            join_path(prefix.lib64.stubs, "libnvidia-ml.so"),
            join_path(prefix.lib64.stubs, "libnvidia-ml.so.1"),
        )
        ...
        shutil.move(
            join_path(prefix.bin, "cuda-gdb"), join_path(prefix.bin, "cuda-gdb.real")
        )
        with open(join_path(prefix.bin, "cuda-gdb"), "w") as f:
            print("#! /bin/bash", file=f)
            print("export PYTHONHOME=" + self.spec["python"].prefix, file=f)
            print(
                'exec {0} "$@"'.format(join_path(prefix.bin, "cuda-gdb.real")), file=f
            )
        ...
        force_symlink(
            ...
            join_path(prefix.bin, "compute-sanitizer"),
        )
        ...
        install(
            join_path(prefix.build.drivers, "libcuda.so.{0}".format(driver_version)),
            prefix.drivers,
        )
        install(
            join_path(
                prefix.build.drivers,
                "libnvidia-ptxjitcompiler.so.{0}".format(driver_version),
            ),
            prefix.drivers,
        )
        ...
        with working_dir(prefix):
            force_symlink(
                "libcuda.so.{0}".format(driver_version),
                join_path("drivers", "libcuda.so.1"),
            )
            force_symlink("libcuda.so.1", join_path("drivers", "libcuda.so"))
            force_symlink(
                "libnvidia-ptxjitcompiler.so.{0}".format(driver_version),
                join_path("drivers", "libnvidia-ptxjitcompiler.so.1"),
            )
            force_symlink(
                "libnvidia-ptxjitcompiler.so.1",
                join_path("drivers", "libnvidia-ptxjitcompiler.so"),
            )
        ...
        filter_file(
            r"\$(_HERE_)", "$(TOP)/bin", join_path(prefix, "bin", "nvcc.profile")
        )
        filter_file(r"/\$(_TARGET_DIR_)", "", join_path(prefix, "bin", "nvcc.profile"))
        filter_file(
            r"\$(_TARGET_SIZE_)", "64", join_path(prefix, "bin", "nvcc.profile")
        )
