...
class Madgraph5amc(Package):
    ...
    variant(
        "golem", default=False, description="Use external installation" + " of Golem"
    )
    variant(
        "syscalc", default=True, description="Use external installation" + "of SysCalc"
    )
    variant(
        "cms",
        default=False,
        description="Apply changes requested by " + "the CMS experimenent on LHC",
    )
    variant(
        "nb_core",
        default=0,
        description="Number of cores to use. Default (0) "
    ...
    def edit(self, spec, prefix):
        ...
        if "+pythia8" in spec:
            ...
            set_parameter(
                "mg5amc_py8_interface_path", spec.prefix.HEPTools.MG5aMC_PY8_interface
            )
        ...
        if "+lhapdf" in spec:
            set_parameter(
                "lhapdf_py2", join_path(spec["lhapdf"].prefix.bin, "lhapdf-config")
            )
            set_parameter(
                "lhapdf_py3", join_path(spec["lhapdf"].prefix.bin, "lhapdf-config")
            )
        ...
        filter_file(
            "SHFLAG = -fPIC",
            "SHFLAG = -fPIC -fcommon",
            join_path(self.stage.source_path, "vendor/StdHEP/src/stdhep_arch"),
        )
    ...
    def build(self, spec, prefix):
        ...
        if "~cms" in spec:
            ...
            shutil.copy(
                "input/mg5_configuration.txt", "input/mg5_configuration_patched.txt"
            )
            # compile_py = Executable(join_path('bin', 'compile.py'))
            py = which("python")
            ...
            shutil.move(
                "input/mg5_configuration_patched.txt", "input/mg5_configuration.txt"
            )
