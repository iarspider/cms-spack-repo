...
class PyTensorflow(Package, CudaPackage):
    ...
    variant(
        "only_python", default=True, description="Only install Python wrapper"
    )  # -- CMS
    ...
    depends_on(
        "py-absl-py@0.10:", type=("build", "run"), when="@2.4.0:"
    ...
    depends_on(
        "py-keras-preprocessing@1.0.3:", type=("build", "run"), when="@1.11:1.11"
    )
    ...
    depends_on(
        "flatbuffers+python@1.12.0:3", when="@2.4.0:", type=("build", "link", "run")
    )
    # tensorboard
    ...
    depends_on(
        "py-typing-extensions@3.7:", type=("build", "run"), when="@2.6.4:"
    )  # CMS: remove upper limit
    depends_on(
        "py-typing-extensions@3.7.4:3.7", type=("build", "run"), when="@2.4.0:2.6.3"
    )
    ...
    def build(self, spec, prefix):
        ...
        if (
            ...
            and self.spec.variants["vectorize_flag"].value
        ):
        ...
        if self.spec.satisfies("target=ppc64le"):
            ...
            args.extend(
                [
                    "--copt=--param=l1-cache-size=64",
                    "--copt=--param=l1-cache-line-size=128",
                    "--copt=--param=l2-cache-size=512",
                ]
            )

        if not (
            self.spec.satisfies("target=ppc64le")
            or self.spec.satisfies("target=x86_64:")
        ):
            args.extend(["--copt=-mcpu=native", "--copt=-mtune=native"])
        ...
        if self.spec.satisfies("~only_python"):  #  -- CMS
            ...
            for entry in glob.glob(
                join_path(
                    self.stage.source_path, "bazel-bin", "tensorflow", "include", "**"
                ),
                recursive=True,
            ):
            ...
            for root, dirs, files in os.walk(
                join_path(self.stage.source_path, "tensorflow")
            ):
                for fn in files:
                    if fn.endswith(".proto"):
                        protoc(
                            "--cpp_out={0}/bazel-bin/tensorflow/include".format(
    ...
    def install(self, spec, prefix):
        ...
        for l in ("tensorflow_cc", "tensorflow_framework", "tensorflow"):
            if not os.path.exists(
                join_path(libdir, "lib{0}.so.{1}".format(l, realversion))
            ):
                continue

            force_symlink(
                join_path(libdir, "lib{0}.so.{1}".format(l, realversion)),
                join_path(libdir, "lib{0}.so.{1}".format(l, majorversion)),
            )
            force_symlink(
                join_path(libdir, "lib{0}.so.{1}".format(l, majorversion)),
                join_path(libdir, "lib{0}.so".format(l)),
            )
        ...
        def copy_headers(arg_1, arg_2):
            root_1 = arg_1.rstrip("/") + "/"
            headers = [
                s.replace(root_1, "") for s in fs.find(join_path(arg_1, arg_2), "*.h")
            ]
