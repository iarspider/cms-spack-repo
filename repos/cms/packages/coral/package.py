...
class Coral(ScramPackage):
    ...
    version(
        "CORAL_2_3_21",
        git="https://github.com/cms-externals/coral",
    ...
    def install(self, spec, prefix):
        ...
        with open(join_path(prefix, "etc", "profile.d", "init.sh"), "w") as f:
            f.write(
                initsh.format(
                    ...
                    pkgversion=str(self.spec.version),
                )
            )
        ...
        if not os.path.exists(join_path(prefix.etc.scramrc, "coral.map")):
            ...
            with open(join_path(prefix.etc.scramrc, "coral.map"), "w") as f:
                f.write(
                    "{ucprojtype}=$SCRAM_ARCH/{pkgname}/{ucprojtype}_*".format(
    ...
    def setup_run_environment(self, spack_env):
        ...
        spack_env.append_path(
            "LD_LIBRARY_PATH", "%s/%s/lib" % (self.prefix, self.cmsplatf)
        )
        spack_env.append_path("PYTHONPATH", "%s/%s/lib" % (self.prefix, self.cmsplatf))
        spack_env.append_path(
            "PYTHONPATH", "%s/%s/python" % (self.prefix, self.cmsplatf)
        )
