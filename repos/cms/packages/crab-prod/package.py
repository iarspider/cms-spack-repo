...
class CrabProd(Package):
    ...
    crab_client_version = "v3.230124"
    crab_client_revision = "00"
    crab_server_version = "v3.221205"
    dbs_version = "3.16.0"
    ...
    version(
        crab_client_version + "." + crab_client_revision,
        sha256="af147dd4a6c715dde7e0f13f4fb990029c13833d73fffc281a09709ce422c46b",
        expand=False,
    )
    ...
    def install(self, spec, prefix):
        ...
        filter_file(
            '"development"',
            '"{0}"'.format(self.spec.version),
            prefix.lib.CRABClient.join("__init__.py"),
        )
        ...
        with open(prefix.etc.join("crab_py_pkgs.txt"), "w") as f:
            for fn in glob.glob("CRABClient/src/python/*/__init__.py"):
                f.write(os.path.basename(os.path.dirname(fn)) + "\n")
            f.write("dbs\n")
            f.write("RestClient\n")
        ...
        install(
            "CRABClient/src/python/CRABClient/WMCoreConfiguration.py", prefix.lib.WMCore
        )
        ...
        if all(flags):
            filter_file(
                "_UseCrab",
                "_UseCrab_" + crab_type,
                prefix.etc.join("crab-bash-completion.sh"),
            )
            filter_file(
                " filenames  *crab",
                " filenames crab-" + crab_type,
                prefix.etc.join("crab-bash-completion.sh"),
            )
            if crab_type == "prod":
