    ...
    def determine_b2_options(self, spec, options):
        ...
        if '+iostreams' in spec:
            options.extend([
                ...
                '-s', 'ZSTD_INCLUDE=%s' % spec['zstd'].prefix.include,
                '-s', 'ZSTD_LIBPATH=%s' % spec['zstd'].prefix.lib,
                '-s', 'LZMA_INCLUDE=%s' % spec['xz'].prefix.include,
                '-s', 'LZMA_LIBPATH=%s' % spec['xz'].prefix.lib,
