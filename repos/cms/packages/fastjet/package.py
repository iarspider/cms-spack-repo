...
class Fastjet(AutotoolsPackage):
    ...
    conflicts(
        "thread-safety=full", when="@:3.3.4", msg="Thread safety was added in 3.4.0"
    )
    conflicts(
        "thread-safety=limited", when="@:3.3.4", msg="Thread safety was added in 3.4.0"
    )
    ...
    def configure_args(self):
        ...
        if self.spec.satisfies("arch=x86_64:"):
            ...
            extra_args.append(
                "CXXFLAGS=-mcpu=power8 -mtune=power8 --param=l1-cache-size=64 --param=l1-cache-line-size=128 --param=l2-cache-size=512"
            )
